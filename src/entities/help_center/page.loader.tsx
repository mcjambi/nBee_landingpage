...
import { TypedPosts } from "queries/page.query";
const helpBanner = require("media/images/help-banner.png");

export default function PageLoader({ slug }) {
  const [loading, setLoading] = useState(true);
  const [data, setData] = useState<TypedPosts>(null);
  let language = localStorage.getItem("language") || "en";

  const loadPage = useCallback(
    async (slug: string) => {
      try {
        let post = await axios.get<TypedPosts>("/posts/" + slug, {
        ...
        <>
          <div className="help-header">
            ...
            <h1 className="post_title">
              {data?.post_to_content?.post_title ?? ""}
            </h1>
          </div>
          <div id="post_wrap">
            <BlockStack gap="500">
              <div id="post_excerpt">
                {data?.post_to_content?.post_excerpt ?? ""}
              </div>
              <div id="post_meta">{pageMeta}</div>
              <div id="post_content">
                {Parser(data?.post_to_content?.post_content ?? " ")}
              </div>
            </BlockStack>
          </div>
        </>
